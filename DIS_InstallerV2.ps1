...
function RunScripts {
    ...
    if($Script:Blocks[6]){
        #Put Script Block 7 code here
        Write-Host "`n Quantum Desktop Shortcut" -ForegroundColor Black -BackgroundColor White
        QuantumShortcut
    }

    if($Script:Blocks[7]){
        #Put Script Block 7 code here
        Write-Host "`n Altura Desktop Shortcut" -ForegroundColor Black -BackgroundColor White
        AlturaShortcut   
    }
}
...
function GenerateForm {
    ...
    $NumofCheckBoxes = 8 #Set the number of checkboxes you would like
    ...
    function QuantumShortcut {
        ...
        $label.Text = 'Enter Quantum Hostname'
        ...
        # 5. Set the Accept button to the OK button so pressing Enter works
        $form.AcceptButton = $okButton

        # 6. Show the form modally and retrieve the input
        [void]$form.ShowDialog()

        # 7. Access the input after the form is closed
        $userInput = $textBox.Text
        $Shell = New-Object -ComObject ("WScript.Shell")
        $Favorite = $Shell.CreateShortcut($env:USERPROFILE + "\Desktop\DIS Quantum.url")
        $Favorite.TargetPath = "https://$userInput.dis.us/webclient"
        $Favorite.Save()
    }
    ...
    function InstallKeystone {
        function CheckDis {
          ...
          function IFSShortcut {
            Write-Host "Creating IFS Desktop shortcut" -ForegroundColor Green
            $lnkPath = "$env:Public\Desktop\DIS IFS SHARE.lnk"
            $wshell = New-Object -ComObject WScript.Shell
            $shortcut = $wshell.CreateShortcut($lnkPath)
            $shortcut.TargetPath = "\\KEYSTONE"
            $shortcut.Save()
            $txtStatus.AppendText("` IFS Shortcut Created`r`n")
            }
          ...
          function StartIfs {
            Write-Host "Sleeping for 15 and mounting the IFS" -ForegroundColor Green
            Start-Sleep -Seconds 15
    ...
    function InstallECCS {
        ...
        Write-Host "Downloading and installing Java32 --silent installer" -ForegroundColor Green
        $ProgressPreference = 'SilentlyContinue'
        ...
        $URI = "https://dis-ts-files.s3.us-west-2.amazonaws.com/Public/im/im41921x.exe"
        $Path= "C:\DIS\im41921x.exe"
        $ProgressPreference = 'SilentlyContinue'
        Invoke-WebRequest -URI $URI -OutFile $Path
        C:\DIS\im41921x.exe
        SetPower   
        ...
        Write-Host "Setting ECCCommand and ECCService as Admin in registry.." -ForegroundColor Green
    ...
    $handler_updateList_Click= {
        ...
        foreach($obj in $CheckBoxes){
            if ($obj.Checked) {
                $listBox1.Items.Add($obj.Text + ' Selected for Install')
                $listBox1.Items.Add('')
    ...
    foreach($obj in $CheckBoxes){
        ...
        if($index -ne 0) { $yPos = ($yPos + $CheckBoxes[$saveIndex].Height) + 6 }
    ...
    $lbXPos = ($CheckBoxes[$saveIndex].Right + 20)
